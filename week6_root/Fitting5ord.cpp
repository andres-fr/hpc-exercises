...
Double_t fitFunction(Double_t *x, Double_t *par) {
  return background(x,par) + lorentzianPeak(x,&par[4]);
}
...
void Fitting5ord() {
 //Bevington Exercise by Peter Malzacher, modified by Rene Brun
   ...
   TF1 *fitFcn = new TF1("fitFcn",fitFunction,0,3,7);
   fitFcn->SetNpx(500);
...
//   fitFcn->SetParameter(5,1);   // peak
   ...
   // improve the picture:
   TF1 *backFcn = new TF1("backFcn",background,0,3,4);
