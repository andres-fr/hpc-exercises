...
/// 
/// use "g++ Matrix.cpp -O3; ./a.out" to run
...
int main() {
  ...
  // 1) copy to array of fvecs
  fvec flatMat[N*N/4];
  for(int i=0; i<N; ++i){
    for(int j=0; j<N/4; ++j){
      for(int dim=0; dim<4; ++dim){
        flatMat[(i*N/4)+j][dim] = a[i][4*j+dim];
      }
    }
  }

  // 2) apply _mm_sqrt_ps to the fvecs
  ...
  // 3) copy back to c_simd
  for(int i=0; i<N; ++i){
    for(int j=0; j<N/4; ++j){
      for(int dim=0; dim<4; ++dim){
        c_simd[i][4*j+dim] = flatMat[i*N/4+j][dim];
      }
    }
  }
