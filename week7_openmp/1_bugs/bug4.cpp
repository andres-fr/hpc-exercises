...
// g++ -O3 -fopenmp -std=c++14 -Wall -Wextra /home/afr/SS17/hpc-praktikum/week7_openmp/1_bugs/bug4.cpp -o bug4 && ./bug4
...
int main() {
  ...
  // like bug3.cpp but with CalculateSum
  float sum = 0;
  {
  ...
  // version 2: this somehow reimplements openMP (no need to work this way).
  // it needed a barrier to avoid race conditions. If using built-in ops, the
  // reduction(+:sum) should be used (see bug3.cpp). This is probably slower??
  sum = 0;
  #pragma omp parallel num_threads(NThreads)
  {
    ...
    #pragma omp barrier // barrier: each thread waits until all threads arrive
