...
int main() {
  ...
  for(int io=0; io<NIterOut; io++)
  {
    for(int i=0; i<NVectors; i++)
    {
      ...
      float_v xV = (-bV+sqrt(det))/(2*aV);

      // copy output data      
      for(int iE=0; iE<float_v::Size; iE++)
        dataSIMD1.data[i*float_v::Size+iE].x = xV[iE];
    }
  }
  timerSIMD1.Stop();
  ...
  for(int io=0; io<NIterOut; io++)
    ...
    {
      float_v& aV = (reinterpret_cast<float_v&>(dataSIMD2.a[i]));
      float_v& bV = (reinterpret_cast<float_v&>(dataSIMD2.b[i]));
      float_v& cV = (reinterpret_cast<float_v&>(dataSIMD2.c[i]));
      float_v& xV = (reinterpret_cast<float_v&>(dataSIMD2.x[i]));
